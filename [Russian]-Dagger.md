...
Идея здесь в том, что не существует никакого способа вычислить `SCRYPT(D)` без построения всего массива `H` и удержания его в памяти до самого конца. Однако у всех scrypt-подобных алгоритмов есть свойство, естественным образом ограничивающее их требовательность к памяти: для верификации некоторого хэша необходимо проделать всю ту же процедуру, так что верификация ровно настолько же требовательна к памяти, как и само вычисление майнера. Таким образом, хотя с помощью алгоритма scrypt и можно добиться требовательности к памяти порядка нескольких мегабайт на поток, требовательность к оперативной памяти порядка сотен мегабайт на поток он не может выдать никак. По-видимому, сотни мегабайт на поток — это то, что поможет индустрии защититься от АСИКов; уже [есть компании](http://www.cryptocoinsnews.com/2013/11/22/alpha-technologies-offer-scrypt-mining-asics/), которые предлагают к продаже scrypt-АСИКи.

### Праймкойн 

Алгоритм [Праймкойна](http://primecoin.io/) не задумывался как требовательный к оперативной памяти, но так вышло, что до некоторой степени таковым является. Майнинг заключается в поиске так называемых цепочек Куннингема — цепочек вида `[ n+1, 2n+1, 4n+1 ..., n*2^k+1 ]` для достаточно больших `k`. Оказывается, что цепочку существенной длины крайне сложно обнаружить без предварительного создания конструкции "решето" (sieve). Преимущество Праймкойна — в том, что верификация происходит практически мгновенно и не предъявляет особых требований к оперативной памяти; всё, что нужно для верификации — запустить [тест Ферма](http://ru.wikipedia.org/wiki/%D0%A2%D0%B5%D1%81%D1%82_%D0%A4%D0%B5%D1%80%D0%BC%D0%B0) для проверки того, что все числа простые. Однако у алгоритма Праймкойна есть две слабости:

1. '''Time-memory tradeoff''' — у АСИКов есть возможность уменьшить требования к памяти, пожертвовав некоторым количеством вычислительной мощности (см. birthday attack ниже); даже памяти в 100 килобайт на поток может быть достаточно для эффективных вычислений.

2. '''All clear effect''' — оказывется, что GPU (=видеокарты) и АСИКи могут выделять разным потокам одну и ту же память. 

### Birthday attack

"Birthday attack" — это следующий остроумный алгоритм:

1. Приравняем `H[i] = sha256(D + N + i)` for `i` from 0 to 2<sup>32</sup> - 1; `D` — информация (например, пачка транзакций), `N` — nonce.
2. Если `abs(H[i] - H[j])` достаточно мало (меньше числа, вычисляемого по сложности), то `(N,i,j)` — валидное решение, хэш найден. 

В чём остроумность? В том, что для подбора подходящего хэша нужно выстраивать в памяти некоторую структуру, при которой каждый новый хэш `H[i]` загружался бы в память и проверялся бы в паре с каждым из старых. А вот для проверки, для валидации решения нужно проверить всего два хэша! Алгоритм не подвержен all-clear effect, т.к. память очищается каждые 2<sup>32</sup> раунда. Однако здесь могут быть лазейки, упрощающие вычисления и time-memory tradeoff attacks. Первая такая лазейка — запоминать хэши не целиком, а только несколько первых байтов. (Т.к. подавляющее большинство из них совсем неблизкие числа => не могут образовывать валидную пару, это позволяет многократно быстрее отсеивать неподходящие.) Вторая — можно запоминать только хэши, начинающиеся с двух битов 00; это сразу сокращает количество возможных решений на 75%, уменьшая эффективность во времени в 4 раза, но также и уменьшает в 4 раза количество необходимой оперативной памяти. Кроме того, возможность оптимизации есть в размещении запоминаемых хэшей; можно хранить их в большом массиве, в бинарном или красно-чёрном (двуцветном?) дереве и других более замысловатых структурах. Оптимальный способ может быть достаточно сложным.

Dagger должен решить все эти проблемы, т.е. быть алгоритмом, который:

1. затратно с точки зрения оперативной памяти посчитать (найти подходящее nonce), но легко с её же точки зрения проверить
2. не имеет all clear effect
3. no time-memory tradeoff even at a 1:1 ratio
4. для которого естественный способ запоминания хэшей является оптимальным. 

## Как устроен алгоритм Dagger
