...
Более проработанная структура имеет также встроенные отдельными пунктами транзакции, регистрирующие голос в поддержку добавления/исключения члена и может даже реализовывать делегирование голосов, подобно тому как это происходит в [облачной демократии] (а происходит это там так: А может передоверить своё право голоса B, и эта операция транзитивна, т.е. если А передоверил свой голос B, а B, в свою очередь, передоверил его C, то С проголосует по голосу участника A). 
...
SchellingCoin работает так: , эти значения сортируются

Таким образом, выгодно отвечать так же, как и большинство, и единственное значение, которое все участники считают средним — объективное. Это даёт децентрализованный протокол, предоставляющий верные значения: курс ETH/USD, температуру в Берлине или даже результат конкретного трудного вычисления.
...
**5. Облачные вычисления.** ВМЭ Эфириума . Это создаёт целый рынок облачных вычислений, в котором может принять участие

(т.е. узлы не могут жульничать с выгодой для себя). Такая система подходит не для всех типов заданий . Все остальные задания, впрочем, намного легче распараллелизовать; проекты наподобие SETI@home, folding@home и генетические алгоритмы с лёгкостью могут быть реализованы на такой платформе.


**6. Пиринговые азартные игры.** Любое количество протоколов пиринговых азартных игр, such as Frank Stajano and Richard Clayton's Cyberdice, может быть включено в блокчейн Эфириума. Простейший такой протокол — контракт на разницу с хэшем следующего блока, и могут быть созданы более продвинутые протоколы сервисов азартных игр с практически нулевыми комиссиями; при этом в таких азартных играх нет никакой возможности жульничества.
