...
(Переменная с точки зрения ЭВМ-кода — это нечто, чему приписывается индекс в памяти; все переменные разделены 32 байтами. В последнем примере `x` — индекс начала массива, который хранится в динамической памяти. Заметим, что проверка "выхода за границы", отведённые переменной, не производится; `array(3)[5] = 10` будет приводить к непредсказуемым и зависящим от компилятора последствиям выполнения кода.) - переписать
