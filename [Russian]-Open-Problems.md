...
* Время выполнения `PoWProduce` должно быть линейно по `diff` (сложности)
* Время выполнения `PoWVerify` должно быть не более чем [полилогарифмическим](http://en.wikipedia.org/wiki/Time_complexity#Polylogarithmic_time) по `diff`
...
* `PoWProduce` должна быть не более чем линейна по вычислительной мощности или времени; именно, 
