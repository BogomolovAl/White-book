...
Запутывание ([обфускация](https://ru.wikipedia.org/wiki/%D0%9E%D0%B1%D1%84%D1%83%D1%81%D0%BA%D0%B0%D1%86%D0%B8%D1%8F)) кода —

Прошло много лет с тех пор, как современные люди овладели искусством криптографии. В основе простых, устойчивых и хорошо тестируемых алгоритмов может лежать как симметричное шифрование (один и тот же ключ используется как для зашифровки, так и расшифровки), так и асимметричное (криптосистема с открытым ключом предполагает, что ключи зашифровки и расшифровки различны и не могут получены один из другого). Однако же, существует еще один вид шифрования, могущий быть потенциально полезным, но для которого на данный момент мы не нашли жизнеспособного алгоритма: алгоритма шифрования программ.

В 2007 г. было доказано, что шифрование по принципу «черный ящик» невозможно — главным образом потому, что между наличием доступа «черного ящика» к программе и наличием кода к этой программе есть существенная разница, независимо от того, насколько они запутаны — мы можем создать определенные классы программ, которые будут противодействовать запутыванию (обфускации). Однако, присутствует вероятность возникновения еще более серьезной проблемы, известной как «неразличимость обфускации». Неразличимость обфускатора `O` возникает, когда вы берете две эквивалентные программы `A` и `B` и принимаете, что `O(A) = P` и `O(B) = Q`, тогда не существует ни одного численно правдоподобного способа для не имеющего доступа ни к `A`, ни к `B` лица установить, откуда пришло `P` — из `A` или из `B`.
Этот тип обфускации может показаться более ограниченным, но, тем не менее, его применение весьма широко. Для того, чтобы убедиться, представьте две программы `F` и `G` при условии, что
- `F` печатает 32-байтную последовательность, хэш которой «12345»;
- `G` считывает хэш «12345» и распечатывает результат. 
Согласно определению неразличимости обфускатора, которое мы дали выше, нет ни одного правдоподобного способа установить, что `O(F)` пришел из `O(G)`. Отсюда, если "12345" может быть возвращен в `O(G)`, тогда для `O(G)` и `O(F)` невозможно установить, что "12345" поступил из `O(F)` — искусный трюк, который влечет слом первичного сопротивления криптографической хэш-функции.
...
Первая из проблем — проблема централизации майнинга — намного тяжелее. Есть некоторая надежда, что со временем она решится сама собой, когда индустрия майнинга разрастётся настолько, что конкурирующих фирм-участников будет много, и ни одна из них не будет иметь достаточно большой процент от рынка. Однако никто не гарантирует, что это произойдёт, и мы должны быть готовы к худшему. Более того, траты на электричество и вычислительные ресурсы, если ничего принципиально не изменится, 

##### 6. Стойкий к АСИКам proof-of-work
...
Третьим подходом к решению проблемы могло бы быть использование ресурса, отличного от вычислительной мощности или криптовалюты. В этом смысле обсуждались две альтернативы — место на диске (storage) и ширина канала (bandwidth). Не существует способа предоставить постфактум криптографическое доказательство того, что bandwidth предоставлялась, была использована, и поэтому proof-of-bandwidth можно использовать, по-видимому, только как один из способов "полезного proof-of-work", который обсуждается чуть ниже в этом документе; но proof-of-storage действительно является чем-то, что может быть вычислительно реализовано. Преимущество proof-of-storage в том, что оно полностью устойчиво к АСИКам; способ хранения файлов, используемый на жёстких дисках, уже очень близок к оптимальному. 
...
```
 ...
 Каждый шаг доказательства может быть проверен
...
В случае с Биткойн (или чем-то подобным) есть простой ответ — успех измеряется вычислительной мощностью сети. Это достаточно эффективный подход, но у него есть два недостатка: во-первых, 

Родственная проблеме децентрализованного измерения вклада — проблема децентрализованного измерения успеха. Как узнать, преуспела ли та или иная организация в достижении поставленных ранее целей? Как узнать, в какой степени она справилась с поставленными задачами? В случае с чем-то подобным Биткойну существует простой, но далекий от совершенства ответ: успех может быть измерен вычислительной мощностью сети. Это достаточно эффективный подход, но у него есть два недостатка: 
во-первых, вычислительная мощность — не лучший заменитель цены, поскольку фактор развития или не развития ИС может исказить результаты; во-вторых, цена, в свою очередь, является не лучшим заменителем успеха, поскольку валюта может иметь большой успех. В случае с финансированием DAO сферы здравоохранения и попыток, направленных на противодействие климатическим изменениям, однако, едва ли можно сказать, что это финансирование имело смысл.
Здесь же издержки накопления информации низки, информация находится в публичном доступе и доступна каждому, что делает возможным достижение высокого уровня точности — будем надеяться, достаточно точного и для осуществления финансовых контрактов. Однако, необходимость поддержания высокого уровня точности (при наличии необходимых финансовых деривативов) порождает новые проблемы. Может ли какая-нибудь достаточно влиятельная организация манипулировать метрикой в своих интересах? Если имеют место издержки накопления информации, подвержена ли система угрозе попадания под централизованный контроль, где каждый вынужден играть по установленным свыше правилам игры?

**Задача:** прийти и рассказать всем, как можно децентрализованно измерять величины, встречающиеся в этом вашем реальном мире.

**Дополнительные требования и допущения:**

* Вовлечёнными сторонами здесь могут быть отдельные люди, группы людей, боты и другие ДАО
* Система должна быть способна измерять*(?)* любые параметры, у которых есть хотя бы что-то похожее на объективное значение (цена актива, температура, концентрация CO<sub>2</sub>)
